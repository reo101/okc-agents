...
async fn handle_connection(
    ...
    Command::new("am")
        .arg("broadcast")
        .arg("-n")
        .arg("org.ddosolitary.okcagent/.SshProxyReceiver")
        .arg("--ei")
        .arg("org.ddosolitary.okcagent.extra.SSH_PROTO_VER")
        .arg(PROTO_VER.to_string())
        .arg("--ei")
    ...
    let mut app_stream = time::timeout(
        Duration::from_secs(10),
        TcpListenerStream::new(app_listener).next(),
    )
    .await
    .map_err(|_| StringError::new("timed out waiting for app to connect"))?
    .unwrap()?;
...
async fn run(listener: StdUnixListener, logger: Logger) -> Result {
    ...
    UnixListenerStream::new(listener)
        .for_each_concurrent(Some(4), |accept_result| async {
...
fn main() {
    ...
    if matches.is_present("kill") {
        let env_pid = std::env::var(AGENT_PID_ENV)
            .ok()
    ...
    *SOCKET_FILE.write().unwrap() = Some(socket_file.clone());
    ...
    let is_foreground =
        cmd.is_some() || matches.is_present("foreground") || matches.is_present("debug");
