...
async fn handle_connection(
    accept_result: std::result::Result<TcpStream, tokio::io::Error>,
    logger: Logger,
) -> Result {
    ...
    let res = match op {
        ...
        _ => Err(
            Box::new(StringError::new("protocol error: invalid connection type")) as Box<dyn Error>,
        ),
    };
    if let Err(e) = res {
        error!(logger, "{:?}", e);
...
async fn run(logger: Logger) -> Result {
    ...
    cmd.arg("broadcast")
        .arg("-n")
        .arg("org.ddosolitary.okcagent/.GpgProxyReceiver")
        .arg("--ei")
    ...
    TcpListenerStream::new(listener)
        .for_each_concurrent(Some(3), |accept_result| async {
